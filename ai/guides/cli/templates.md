...
[defineTemplate](/ai/api/cli/types/utils/function-define-template.md) (exported from `@tsed/cli`) registers reusable generators that appear under `tsed generate`. Templates can prompt for custom data, validate it, and return string content or richer render instructions that the CLI writes to disk.
