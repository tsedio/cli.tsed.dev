...
2.  Each task receives `(ctx, logger)` arguments—`ctx` is your command context, `logger` is a [TaskLogger](/ai/api/cli-tasks/types/domain/class-task-logger.md).
...
The following examples perform package installation, stream child-process output, and emit nested progress bars—one using [Command](/ai/api/cli-core/types/decorators/decorator-command.md), the other using [command](/ai/api/cli-core/types/fn/function-command.md):
...
Set the `type` field on a task to control which renderer [TaskLogger](/ai/api/cli-tasks/types/domain/class-task-logger.md) uses. Supported values match `TaskLoggerOptions["type"]` (see [Task](/ai/api/cli-tasks/types/interfaces/interface-task.md)).
