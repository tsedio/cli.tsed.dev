...
`@tsed/cli-prompts` wraps `@clack/prompts` so every question runs inside the Ts.ED injector via the [PromptRunner](/ai/api/cli-prompts/types/class-prompt-runner.md) service. That means you can inject services, reuse providers, and share state while guiding users through generator wizards.

## Prompt types

Define prompts by returning a `PromptQuestion[]` from the `$prompt` hook (decorators) or the `prompt()` option on [command](/ai/api/cli-core/types/fn/function-command.md). Each question sets its `type` field to one of the following:
...
Use synchronous or async validation functions to reject invalid answers, then branch on the results with `when`. The snippets below show the same interactive command implemented with [Command](/ai/api/cli-core/types/decorators/decorator-command.md) and [command](/ai/api/cli-core/types/fn/function-command.md) so prompts, tasks, and execution all live together:
